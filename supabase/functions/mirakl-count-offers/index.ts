// Mirakl Count Offers Edge Function
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
import { corsHeaders } from '../_shared/cors.ts';
import { getOffersCount } from '../_shared/mirakl-offers.ts';
...
console.log("Mirakl Count Offers Edge Function starting with URL:", SUPABASE_URL);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing offer count request for store ID: ${storeId}`);
    ...
    // Call the getOffersCount function to get the number of offers in Mirakl
    const count = await getOffersCount(domain, apiKey);
    
    return new Response(
      JSON.stringify({ 
        success: true, 
        count: count,
        message: `Found ${count} offers in Mirakl`
      }),
