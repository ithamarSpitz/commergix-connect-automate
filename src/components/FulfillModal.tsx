...
export function FulfillModal({
  ...
  useState(() => {
    if (isOpen) {
      setSelectedItems(
        orderItems
          .filter((item) => item.fulfillment_status === "unfulfilled")
          .map((item) => item.id)
      );
  ...
  const handleFulfill = async () => {
    ...
    try {
      ...
      const { data, error } = await supabaseClient.functions.invoke("create-shipment", {
        body: {
          name: "אוריה פרל",
          city: "תל אביב",
          phone: "0521234567",
      ...
      toast({
        ...
        description: data || "המשלוח נשלח בהצלחה דרך צ'יטה",
      });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => {
              console.log("הכפתור Create Shipment נלחץ!");
              handleFulfill();
            }}
